...
const DIAGNOSTICO_FLOW_ID = Number(process.env.CRM_DIAGNOSTICO_FLOW_ID ?? "0");
...
async function loadConfig() {
  ...
  const flow = flows.find((f: any) => f.id === DIAGNOSTICO_FLOW_ID);
  if (!flow) throw new Error(`Flow ${DIAGNOSTICO_FLOW_ID} no encontrado`);
...
async function createOpportunity(
  ...
  if (!res.ok) {
    ...
    throw new Error(err?.message || "Error al registrar diagnóstico");
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    await createOpportunity(personId, orgId, `Diagnóstico: ${name}`, notes ?? "");
